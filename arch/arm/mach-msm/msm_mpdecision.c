 ...
 * modified by mrg666 - https://github.com/mrg666 - 2013/07/09
 *
...
static int mp_decision(void) {
	...
	if (msm_mpdec_tuners_ins.min_cpus == nr_cpu_ids)
		return MSM_MPDEC_IDLE;
				...
				const char *buf, size_t count)
{
	...
	if ((input < 1) || (input > nr_cpu_ids) || 
		...
		msm_mpdec_tuners_ins.max_cpus = nr_cpu_ids;
	else
